...
import { getHistories } from 'apiSdk/histories';
...
import { ArticleKeywordListPage as ShowCaseList } from 'pages/article-keywords';

function DashboardPage() {
  const { session } = useSession();

  const historyStat = useSWR<PaginatedInterface<HistoryInterface>>(
    () => `/histories`,
    () => getHistories({ limit: 1, offset: 0 }),
  );
  ...
  const keywordStat = useSWR<PaginatedInterface<KeywordInterface>>(
    () => `/keywords`,
    () => getKeywords({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Histories',
      result: historyStat?.data?.totalCount,
      isLoading: historyStat.isLoading,
    ...
    {
      title: 'Keywords',
      result: keywordStat?.data?.totalCount,
      isLoading: keywordStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Article Keywords
                </Text>
                <NextLink href={`article-keywords`} passHref legacyBehavior>
                  <Button
