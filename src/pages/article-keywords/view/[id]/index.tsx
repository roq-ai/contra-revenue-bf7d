...
import { getArticleKeywordById } from 'apiSdk/article-keywords';
import { ArticleKeywordInterface } from 'interfaces/article-keyword';

function ArticleKeywordViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                <Box>
                  ...
                  >
                    Article Keyword Details
                  </Text>
                </Box>
                {hasAccess('article_keyword', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/article-keywords/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('keyword', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Keyword"
                    text={
                      <Link as={NextLink} href={`/keywords/view/${data?.keyword?.id}`}>
                        {data?.keyword?.word}
                      </Link>
                    }
                  />
