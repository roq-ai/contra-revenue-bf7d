...
type ColumnType = ColumnDef<KeywordInterface, unknown>;
...
export function KeywordListPage(props: KeywordListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getKeywords({
        relations: [, 'article_keyword.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['word.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<KeywordInterface>>(
    () => `/keywords?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: KeywordInterface) => {
    if (hasAccess('keyword', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/keywords/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'word', header: 'Word', accessorKey: 'word' },
    hasAccess('article_keyword', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'article_keyword',
          header: 'Article Keyword',
          accessorKey: 'article_keyword',
          cell: ({ row: { original: record } }: any) => record?._count?.article_keyword,
        }
      : null,
